      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mt-4">
        ...
        {editBox === 'specialties' ? (
          <div className="bg-white rounded-lg shadow p-6 relative col-span-2">
            ...
            >
              <div className="bg-blue-50 rounded-lg p-4 space-y-2 shadow-inner grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-3">
                {SPECIALTIES.map((option) => (
                  <label key={option} className={`flex items-center gap-3 px-4 py-3 rounded-lg cursor-pointer transition-all duration-200 font-semibold tracking-wide shadow-sm bg-white hover:bg-blue-50 hover:scale-105 focus-within:ring-2 focus-within:ring-blue-400 min-w-0 ${coach.specialties.includes(option) ? 'bg-blue-100 text-blue-800 ring-2 ring-blue-400' : 'text-gray-800'}`}
                ...
                >
                  ...
                  />
                  <span className="transition-all duration-200 text-sm break-words flex-1 min-w-0 leading-relaxed">{option}</span>
                </label>
                ))}
              </div>
            </div>
            ...
            >
              <div className="bg-blue-50 rounded-lg p-4 space-y-2 shadow-inner grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-3">
                {CREDENTIALS.map((option) => (
                  <label key={option} className={`flex items-center gap-3 px-4 py-3 rounded-lg cursor-pointer transition-all duration-200 font-semibold tracking-wide shadow-sm bg-white hover:bg-blue-50 hover:scale-105 focus-within:ring-2 focus-within:ring-blue-400 min-w-0 ${coach.credentials.includes(option) ? 'bg-blue-100 text-blue-800 ring-2 ring-blue-400' : 'text-gray-800'}`}
                  style={{ userSelect: 'none' }}
                >
                  <input
                    type="checkbox"
            ...
            >
              <div className="bg-blue-50 rounded-lg p-4 space-y-2 shadow-inner grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-3">
                {DIVISIONS.map((option) => (
                  <label key={option} className={`flex items-center gap-3 px-4 py-3 rounded-lg cursor-pointer transition-all duration-200 font-semibold tracking-wide shadow-sm bg-white hover:bg-blue-50 hover:scale-105 focus-within:ring-2 focus-within:ring-blue-400 min-w-0 ${coach.divisions.includes(option) ? 'bg-blue-100 text-blue-800 ring-2 ring-blue-400' : 'text-gray-800'}`}
                  style={{ userSelect: 'none' }}
                >
                  <input
                    type="checkbox"
            ...
            >
              <div className="bg-blue-50 rounded-lg p-4 space-y-2 shadow-inner grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-3">
                {CLIENT_TYPES.map((option) => (
                  <label key={option} className={`flex items-center gap-3 px-4 py-3 rounded-lg cursor-pointer transition-all duration-200 font-semibold tracking-wide shadow-sm bg-white hover:bg-blue-50 hover:scale-105 focus-within:ring-2 focus-within:ring-blue-400 min-w-0 ${coach.clientTypes.includes(option) ? 'bg-blue-100 text-blue-800 ring-2 ring-blue-400' : 'text-gray-800'}`}
                  style={{ userSelect: 'none' }}
                >
                  <input
                    type="checkbox"
            ...
            >
              <div className="bg-blue-50 rounded-lg p-4 space-y-2 shadow-inner grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-3">
                {FEDERATIONS.map((option) => (
                  <label key={option} className={`flex items-center gap-3 px-4 py-3 rounded-lg cursor-pointer transition-all duration-200 font-semibold tracking-wide shadow-sm bg-white hover:bg-blue-50 hover:scale-105 focus-within:ring-2 focus-within:ring-blue-400 min-w-0 ${coach.federations.includes(option) ? 'bg-blue-100 text-blue-800 ring-2 ring-blue-400' : 'text-gray-800'}`}
                  style={{ userSelect: 'none' }}
                >
                  <input
                    type="checkbox"
