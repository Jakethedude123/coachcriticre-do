...
async function checkAndFixCoachData() {
  try {
    console.log('🔍 Checking for coaches with incorrect option names...');
    ...
    for (const docSnapshot of querySnapshot.docs) {
      ...
      console.log(`\n📋 Checking coach: ${coachData.name || 'Unknown'} (ID: ${coachId})`);
...
async function checkCoachDataOnly() {
  try {
    console.log('🔍 Checking for coaches with incorrect option names (read-only)...');
      ...
      for (const [fieldName, correctOptions] of Object.entries(CORRECT_OPTIONS)) {
        ...
        if (currentOptions.length > 0) {
          ...
          for (const option of currentOptions) {
            // Check if this option needs to be mapped
            if (mappings[option.toLowerCase()]) {
              console.log(`    ❌ Found old option: "${option}" -> should be "${mappings[option.toLowerCase()]}"`);
              hasIssues = true;
            }
    ...
    if (coachesWithIssues === 0) {
      ...
      console.log(`\n💡 Run the script with fix=true to automatically fix these issues.`);
    }
